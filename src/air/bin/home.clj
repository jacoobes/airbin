...
(defn select-lang [{:keys [disabled selected] :or {disabled false selected "txt"}}] 
    [:select {:id "cars" :name "lang" :disabled disabled
...
(def base-btn-style '[inline-flex border bg-teal-600 hover:bg-teal-600 text-white
                      rounded py-2 px-4 mr-2 border-transparent shadow-sm items-center])
  ...
  (biff/form
   {:id "code" :action "/api/upload" :class "flex flex-auto" :hidden {:on-error "/" }}
    ...
    [:div { :class "absolute bottom-2 right-2  flex items-center" } 
      (select-lang {})
      [:button { :type "submit" :class base-btn-style} 
       "Submit"]]
...
(defn content [{:keys [path-params biff/db] :as ctx}]
  (let [{:keys [post/text post/lang] :as doc} (lookup db :xt/id (parse-uuid (:id path-params)))]
    (if (some? doc)
        (ui/page 
          [:div {:class '[flex flex-grow w-full h-full]}
            ...
            [:div { :class "absolute bottom-2 right-2 flex items-center" } 
              ...
              [:button.cpy { :class base-btn-style
                             :onclick "(document.getElementById('codearea').select(),document.execCommand('copy'),alert('copied'))" } "Copy"]
              [:button { :class  base-btn-style
